...
func init() {
	...
	if len(binRuntimeENSStr) != 0 {
		binRuntimeENS, err = hex.DecodeString(binRuntimeENSStr[2:])
		if err != nil {
			panic(fmt.Errorf("cannot parse ENS bin runtime: %v", err))
		}
	}
}
