...
func (self *message) From() ethgo.Address {
	return self.from
}

func (self *message) To() *ethgo.Address {
	return self.to
}
...
func (self *message) CheckNonce() bool {
	return self.checkNonce
}
func (self *message) Data() []byte {
	return self.data
}

func MessageFromTx(tx *ethgo.Transaction, checkNonce bool) Message {
	value := tx.Value
	if value == nil {
		value = big.NewInt(0)
	}
...
func NewStateTransition(evm *evm.EVM, msg Message, feeReceiver ethgo.Address) *StateTransition {
...
func (st *StateTransition) to() ethgo.Address {
	...
	if st.msg.CheckNonce() {
		...
		if msgNonce := st.msg.Nonce(); stNonce < msgNonce {
			return fmt.Errorf("%w: address %v, tx: %d state: %d", ErrNonceTooHigh,
				st.msg.From().String(), msgNonce, stNonce)
...
func (st *StateTransition) TransitionDb() (*ethgo.ExecutionResult, error) {
	...
	if st.gas < gas {
		...
		vmerr = fmt.Errorf("%w: address %v", ErrInsufficientFundsForTransfer, msg.From().String())
	}
	st.gas -= gas
	if vmerr == nil {
		if contractCreation {
