...
type Storage map[web3.Hash]web3.Hash
...
type StructLog struct {
	...
	Storage       map[web3.Hash]web3.Hash `json:"-"`
...
type Tracer interface {
	CaptureStart(from web3.Address, to web3.Address, create bool, input []byte, gas uint64, value *big.Int)
...
type StructLogger struct {
	...
	storage map[web3.Address]Storage
	logs    []StructLog
	output  []byte
	err     error
}
...
func NewStructLogger(cfg *LogConfig) *StructLogger {
	logger := &StructLogger{
		storage: make(map[web3.Address]Storage),
	}
	if cfg != nil {
		logger.cfg = *cfg
	}
...
func (l *StructLogger) CaptureStart(from web3.Address, to web3.Address, create bool, input []byte, gas uint64, value *big.Int) {
}
...
func (l *StructLogger) CaptureState(env *EVM, pc uint64, op OpCode, gas, cost uint64, memory *Memory,
	...
	if !l.cfg.DisableStorage {
		...
		if op == SLOAD && stack.len() >= 1 {
			var (
				address = web3.Hash(stack.data[stack.len()-1].Bytes32())
		...
		if op == SSTORE && stack.len() >= 2 {
			var (
				value   = web3.Hash(stack.data[stack.len()-2].Bytes32())
				address = web3.Hash(stack.data[stack.len()-1].Bytes32())
			)
			l.storage[contract.Address()][address] = value
		}
...
func (t *mdLogger) CaptureStart(from web3.Address, to web3.Address, create bool, input []byte, gas uint64, value *big.Int) {
	if !create {
