...
func (self *ZeroCopySource) NextAddress() (data ethgo.Address, eof bool) {
	var buf []byte
	buf, eof = self.NextBytes(ethgo.AddressLength)
	if eof {
		return
	}
	copy(data[:], buf)
...
func (self *ZeroCopySource) NextHash() (data ethgo.Hash, eof bool) {
	var buf []byte
	buf, eof = self.NextBytes(ethgo.HashLength)
	if eof {
		return
	}
	copy(data[:], buf)
